...
func ShareSend(ctx context.Context, p *party.HonestParty, ID []byte, current bool, F uint32, N uint32, secret bls.Fr) ([]byte, []byte) {
	...
	vsFr := make([]kbls.Fr, N)
	for i := uint32(0); i < N; i++ {
		str := sha256.Sum256([]byte(vs[i].String()))
		var bv big.Int
		bv.SetString(hex.EncodeToString(str[:]), 16)
		vsFr[i] = *kbls.NewFr().RedFromBytes(bv.Bytes())
	}
	VCvs := p.VC.Commit(vsFr)
...
func verifyWpAcssSend(p *party.HonestParty, vDec *party.VShare, pDec *party.PiShare) bool {
	...
	if p.VC.Verify(pDec.VCvs, *GsiFr, int(p.PID), pDec.PrfRec.PiDpki) {
	...
	if !dprf.VrfyKey(p, p.PID, vDec.DskShare, pDec.PrfRec.Dpki, pDec.PrfRec.PCdsk, pDec.PrfRec.VCdpk, pDec.PrfRec.Wdski, pDec.PrfRec.PiDpki) {
...
func genRecPoly(p *party.HonestParty, ID []byte, f uint32, n uint32) ([]bls.Fr, [][]bls.Fr, []party.ProofRec) {
	...
	for i := uint32(0); i < n; i++ {
		...
		y[i] = *(*bls.Fr)(utils.HashG1ToFr(&tmp))
...
func encapsulateWpAcssSend(v *party.VShare, pi *party.PiShare, FLGmdEncoded bool) ([]byte, []byte) {
	...
	// }
	msg.P.ProofRec.PiDpki = pi.PrfRec.PiDpki
...
func decapAndVrfyWpAcssSend(p *party.HonestParty, m *protobuf.WpAcssShare) (*party.VShare, *party.PiShare, []byte, bool) {
	...
	// }
	piDec.PrfRec.PiDpki = m.P.ProofRec.PiDpki
